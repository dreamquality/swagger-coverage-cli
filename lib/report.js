...
/**
 * Calculate negative testing coverage metrics
 */
function calculateNegativeTestingMetrics(coverageItems) {
  ...
  // Calculate error status code distribution
  const errorDistribution = {};
  negativeStatusCodes.forEach(code => {
    const opsForCode = negativeOps.filter(item => item.statusCode === code);
    const matchedForCode = opsForCode.filter(item => !item.unmatched).length;
    if (opsForCode.length > 0) {
      errorDistribution[code] = {
        total: opsForCode.length,
        matched: matchedForCode,
        coverage: (matchedForCode / opsForCode.length) * 100
      };
    }
  });
  ...
  return {
    positive: {
      total: positiveOps.length,
      matched: positiveMatched,
      coverage: positiveCoverage
    },
    negative: {
      total: negativeOps.length,
...
function generateHtmlReport({ coverage, coverageItems, meta }) {
  ...
  // Calculate negative testing metrics
  ...
  const negativeMetricsJson = JSON.stringify(negativeMetrics);
...
<header>
  ...
  <div class="meta-info">
    ...
    <div class="negative-testing-summary">
      <p><strong>Positive Test Coverage:</strong> ${negativeMetrics.positive.coverage.toFixed(1)}% (${negativeMetrics.positive.matched}/${negativeMetrics.positive.total})</p>
      <p><strong>Negative Test Coverage:</strong> ${negativeMetrics.negative.coverage.toFixed(1)}% (${negativeMetrics.negative.matched}/${negativeMetrics.negative.total})</p>
    </div>
  </div>
</header>
...
<section class="negative-testing-section">
  <h2>🧪 Negative Testing Analysis</h2>
...
<script>
  ...
  function renderNegativeTestingMetrics() {
    ...
    if (recommendations.length === 0) {
      recHtml += '<li class="rec-success">✅ Good negative test coverage detected!</li>';
    } else {
