...
/**
 * generateHeatmapReport - creates a graph visualization showing:
 *   - Nodes: API endpoints
 *   - Edges: HTTP methods connecting endpoints
 *   - Coverage highlighting: color-coded based on test coverage
 */
function generateHeatmapReport({ coverage, coverageItems, meta }) {
  const { timestamp, specName, postmanCollectionName, apiCount = 1, apiNames = [] } = meta;
  
  // Process coverage data to create nodes and edges
  const nodes = [];
  const edges = [];
  ...
  coverageItems.forEach(item => {
    ...
    pathGroups.get(item.path).push(item);
  });
  
  // Create nodes for each unique path
  let nodeId = 0;
  pathGroups.forEach((methods, path) => {
    const coveredMethods = methods.filter(m => !m.unmatched);
    const coverageRatio = coveredMethods.length / methods.length;
    
    // Color based on coverage: green for full coverage, yellow for partial, red for none
    let color = '#f44336'; // red for no coverage
    if (coverageRatio === 1) {
      color = '#4caf50'; // green for full coverage
    } else if (coverageRatio > 0) {
      color = '#ff9800'; // orange for partial coverage
    }
    ...
    const node = {
      ...
      color: color,
      size: 20 + (methods.length * 5), // Size based on number of methods
  ...
  // Create edges between paths that share similar patterns or tags
  const pathArray = Array.from(pathGroups.keys());
  for (let i = 0; i < pathArray.length; i++) {
    for (let j = i + 1; j < pathArray.length; j++) {
      const path1 = pathArray[i];
      const path2 = pathArray[j];
      
      // Connect paths that share common segments or tags
      ...
      if (commonSegments.length > 0) {
        // Create edge with weight based on common segments
        edges.push({
          from: nodeMap.get(path1),
          to: nodeMap.get(path2),
          weight: commonSegments.length,
          title: `Common segments: ${commonSegments.join(', ')}`
        });
      }
    }
  }
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>API Coverage Heatmap - ${specName}</title>
  ...
  <style>
    ...
    * {
    ...
    .legend-color.covered { background: #4caf50; }
    .legend-color.partial { background: #ff9800; }
    .legend-color.uncovered { background: #f44336; }
...
<body>
  <div class="header">
    <h1>API Coverage Heatmap</h1>
    <div class="header-info">
      <span><strong>Specification:</strong> ${specName}</span>
      <span><strong>Collection:</strong> ${postmanCollectionName}</span>
      <span><strong>Generated:</strong> ${timestamp}</span>
      <span><strong>Overall Coverage:</strong> ${coverage.toFixed(2)}%</span>
    </div>
  </div>
  ...
  <div class="controls">
    ...
    <div style="display: flex; gap: 0.5rem;">
      <button class="button secondary" onclick="resetGraph()">Reset View</button>
      <button class="button secondary" onclick="togglePhysics()">Toggle Physics</button>
  ...
  <div class="stats-container">
    <div class="stat-item">
      <div class="stat-value">${coverageItems.length}</div>
      <div class="stat-label">Total Endpoints</div>
    </div>
    <div class="stat-item">
      <div class="stat-value">${coverageItems.filter(item => !item.unmatched).length}</div>
      <div class="stat-label">Covered Endpoints</div>
    </div>
    <div class="stat-item">
  ...
  <footer>
    <p>Generated by swagger-coverage-cli (Heatmap Mode)</p>
  </footer>
  ...
  <script>
    ...
    const coverageItems = ${JSON.stringify(coverageItems)};
    ...
    function createHeatmapMatrix() {
      ...
      // Create a matrix visualization showing paths vs methods
      const paths = [...new Set(coverageItems.map(item => item.path))];
      const methods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD', 'OPTIONS'];
      
      // Create a grid for the heatmap
      const gridHtml = \`
        <div style="padding: 20px;">
          <h3 style="margin-bottom: 20px; text-align: center;">API Coverage Matrix</h3>
          <div style="display: grid; grid-template-columns: 200px repeat(\${methods.length}, 1fr); gap: 2px; font-size: 12px;">
            <div style="grid-column: 1; background: var(--md-surface-color); padding: 8px; font-weight: bold; border: 1px solid #ddd;">Path</div>
            \${methods.map(method => \`<div style="background: var(--md-surface-color); padding: 8px; font-weight: bold; text-align: center; border: 1px solid #ddd;">\${method}</div>\`).join('')}
            \${paths.map(path => {
              const pathMethods = coverageItems.filter(item => item.path === path);
              return \`
                <div style="background: var(--md-surface-color); padding: 8px; border: 1px solid #ddd; word-break: break-all;" title="\${path}">\${path}</div>
                \${methods.map(method => {
                  const item = pathMethods.find(pm => pm.method.toLowerCase() === method.toLowerCase());
                  let bgColor = '#f0f0f0'; // default gray
                  let title = \`\${method} \${path} - Not defined\`;
                  
                  if (item) {
                    if (!item.unmatched) {
                      bgColor = '#4caf50'; // green for covered
                      title = \`\${method} \${path} - Covered (\${item.matchedRequests.length} test\${item.matchedRequests.length > 1 ? 's' : ''})\`;
                    } else {
                      bgColor = '#f44336'; // red for uncovered
                      title = \`\${method} \${path} - Not covered\`;
                    }
                  }
                  
                  return \`<div style="background: \${bgColor}; border: 1px solid #ddd; min-height: 30px; cursor: pointer;" title="\${title}" onclick="showCellDetails('\${method}', '\${path}')"></div>\`;
                }).join('')}
              \`;
            }).join('')}
          </div>
          ...
          <div style="margin-top: 20px; text-align: center;">
            <div style="display: inline-flex; gap: 15px; align-items: center;">
              ...
              <div style="display: flex; align-items: center; gap: 5px;">
                <div style="width: 20px; height: 20px; background: #f0f0f0;"></div>
    ...
    function createEndpointsList() {
      // Create an additional detailed list view
      const listContainer = document.createElement('div');
      listContainer.style.cssText = 'margin: 20px; padding: 20px; background: var(--md-surface-color); border-radius: 8px;';
      
      const title = document.createElement('h3');
      title.textContent = 'Endpoint Details';
      ...
      coverageItems.forEach(item => {
        ...
        itemDiv.style.cssText = \`
          ...
          margin: 5px 0; 
          border-radius: 4px; 
          background: \${!item.unmatched ? '#e8f5e8' : '#ffebee'};
          border-left: 4px solid \${!item.unmatched ? '#4caf50' : '#f44336'};
        \`;
        
        itemDiv.innerHTML = \`
          <span style="background: \${getMethodColor(item.method)}; color: white; padding: 4px 8px; border-radius: 3px; font-size: 11px; font-weight: bold; min-width: 60px; text-align: center; margin-right: 10px;">
            \${item.method}
          </span>
          <span style="flex: 1; font-family: monospace; margin-right: 10px;">\${item.path}</span>
          <span style="margin-right: 10px;">\${item.name || ''}</span>
          <span style="color: \${!item.unmatched ? '#2e7d2e' : '#c62828'}; font-weight: bold;">
            \${!item.unmatched ? \`✓ Covered (\${item.matchedRequests.length} test\${item.matchedRequests.length > 1 ? 's' : ''})\` : '✗ Not Covered'}
          </span>
        \`;
      ...
      document.querySelector('.graph-container').appendChild(listContainer);
    }
    ...
    function showCellDetails(method, path) {
      const item = coverageItems.find(item => item.method.toLowerCase() === method.toLowerCase() && item.path === path);
      
      if (item) {
        let details = \`Path: \${path}\\nMethod: \${method}\\n\\n\`;
        
        if (!item.unmatched) {
          details += \`Status: ✓ COVERED\\nTests: \${item.matchedRequests.length}\\n\\n\`;
          details += \`Matched Requests:\\n\`;
          item.matchedRequests.forEach((req, idx) => {
            details += \`  \${idx + 1}. \${req.name}\\n\`;
          });
        } else {
          details += \`Status: ✗ NOT COVERED\\nNo matching tests found.\`;
        }
    ...
    function resetGraph() {
      // Scroll to top of matrix
      document.querySelector('.graph-container').scrollTo(0, 0);
    }
    
    function togglePhysics() {
      // Not applicable for matrix view
      alert('Physics toggle is not applicable for matrix visualization');
    }
    ...
    function toggleTheme() {
      ...
      // Recreate the matrix with new theme
      createHeatmapMatrix();
    }
...
module.exports = { generateHtmlReport, generateHeatmapReport };
