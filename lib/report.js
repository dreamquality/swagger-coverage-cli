...
/**
 * generateGraphHtmlReport - generates a graph visualization of API coverage
 * using D3.js to show nodes (endpoints) and edges (dependencies) with color coding
 */
function generateGraphHtmlReport({ coverage, coverageItems, meta }) {
  ...
  coverageItems.forEach((item, index) => {
    ...
    const node = {
      ...
      group: item.tags && item.tags.length > 0 ? item.tags[0] : 'default'
    };
    
    nodes.push(node);
    nodeMap.set(`${item.method}-${item.path}`, node);
  });

  // Create edges based on path relationships (simple heuristic)
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      const nodeA = nodes[i];
      const nodeB = nodes[j];
      
      // Create links between nodes that share similar paths or are in the same group
      if (nodeA.group === nodeB.group && nodeA.group !== 'default') {
        links.push({
          source: nodeA.id,
          target: nodeB.id,
          type: 'same-group'
        });
      } else if (nodeA.path.includes(nodeB.path) || nodeB.path.includes(nodeA.path)) {
        links.push({
          source: nodeA.id,
          target: nodeB.id,
          type: 'path-related'
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>API Coverage Graph Report</title>
...
<body>
  <header>
    <h1>API Coverage Graph Report</h1>
    <div class="meta-info">
      <p><strong>Timestamp:</strong> ${timestamp}</p>
      <p><strong>API Spec${apiCount > 1 ? 's' : ''}:</strong> ${specName}</p>
      ${apiCount > 1 ? `<p><strong>Individual APIs:</strong> ${apiNames.join(', ')}</p>` : ''}
  ...
  <footer>
    <p>Generated by swagger-coverage-cli - Graph visualization shows API endpoints as nodes with coverage status color coding</p>
  </footer>
    ...
    function showTooltip(event, d) {
      ...
      tooltip
        ...
        .html(\`
          ...
          Method: \${d.method}<br/>
          Path: \${d.path}<br/>
          Status: \${d.status.replace('-', ' ')}<br/>
          \${d.apiName ? \`API: \${d.apiName}<br/>\` : ''}
          Group: \${d.group}
        \`);
    }
